...
1. Iniciar o **AWS Academy Learner Lab** e copiar as credenciais temporárias (`AWS Details`).
2. Abrir o notebook `atividade2_datalake.ipynb` no **Google Colab**.
3. Colar as credenciais na célula de configuração da sessão `boto3` (região `us-east-1`).
4. Executar as células na ordem, de cima para baixo:
...
5. As duas queries de auditoria (metadados e conciliação de integridade) foram executadas diretamente no **console web do Amazon Athena** — ver evidências abaixo.
...
```
Evidência: [`evidencias/athena_metadados.png`](evidencias/athena_metadados.png)
...
Evidência: [`evidencias/athena_conciliacao.png`](evidencias/athena_conciliacao.png)
